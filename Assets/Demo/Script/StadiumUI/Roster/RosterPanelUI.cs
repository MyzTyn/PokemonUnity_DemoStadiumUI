using System.Collections.Generic;
using UnityEngine;
using UnityEngine.UI;

[ExecuteInEditMode]
public class RosterPanelUI : MonoBehaviour
{
	public float CurrentSrollPosition { get { return scrollBar.value; } set { scrollBar.value = value; } }
	[SerializeField] private Scrollbar scrollBar;
	//on awake, get component, set vertical scrollbar
	//private ScrollRect scrollRect; 

	/*[SerializeField] private GameObject buttonTemplate;
	[SerializeField] private GridLayoutGroup gridGroup;
	[SerializeField] private DemoStadiumManager demo;
	[SerializeField] private GameObject RentalUI;
	//private List<Pokemons> ID;
	public void DisplayPokemonSelectUI()
	{
		//ID = new List<Pokemons>();
		//for (int i = 1; i <= 151; i++)
		//{
		//	ID.Add(i);
		//}
		GetPokemonRental();
		ActiveRentalUI(true);
	}
	private void GetPokemonRental()
	{
		//if (ID.Count < 4)
		//{
		//	gridGroup.constraintCount = ID.Count;
		//}
		//else
		//{
		//	gridGroup.constraintCount = 3;
		//}
		//foreach (int id in ID)
		for (int id = 1; id <= 151; id++)
		{
			GameObject Button = Instantiate(buttonTemplate);
			Button.SetActive(true);
			demo.RentalData(id, Button.GetComponent<RosterEntryButton>());
			Button.GetComponent<RosterEntryButton>().SetID(id);
			Button.transform.SetParent(buttonTemplate.transform.parent, false);
		}
	}
	public void ActiveRentalUI(bool active)
	{
		RentalUI.SetActive(active);
	}*/
}