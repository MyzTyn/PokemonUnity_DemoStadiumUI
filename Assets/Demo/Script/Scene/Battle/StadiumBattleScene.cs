...
{
    public class StadiumBattleScene : MonoBehaviour, IPokeBattle_SceneIE
    {
	    private BattleResults Result = BattleResults.InProgress;
		...
		private void Awake()
		{
			if (commandwindow == null) 
				commandwindow = GetComponentInChildren<ICommandMenuDisplay>();
			if (fightwindow == null) 
				fightwindow = GetComponentInChildren<IFightMenuDisplay>();
		...
		private void Start()
		{
			...
			IPokemon[] p2 = PokemonSelect.TemporaryParty.Count != 0 ? PokemonSelect.TemporaryParty.Take(1).ToArray() : GenerateRandomParty().Take(1).ToArray();
			...
			var result = battle.StartBattle(true);

			StartCoroutine(GoToRosterSelection());
		}
		#endregion
        ...
        // This is for demo only. In case if you only click battle scene instead of Stadium Demo
        private IPokemon[] GenerateRandomParty()
        {
            ...
            return Enumerable.Range(0, Core.MAXPARTYSIZE)
                .Select(_ => 
                {
                    var pokemon = new PokemonUnity.Monster.Pokemon((Pokemons)rnd.Next(151) + 1, level: 50);
        ...
        // ToDo: Fix the battle. It cannot be stop
        private IEnumerator GoToRosterSelection()
        {
	        Result = BattleResults.InProgress;
	        battle.StartBattle(true);

	        // Wait until the result is no longer 'InProgress'
	        yield return new WaitUntil(() => Result != BattleResults.InProgress);

	        // Load the Intro scene once the battle result changes
	        GameManager.current.OnLoadLevel((int)Scenes.RosterSelection);
        }
    }
}